...
test.describe("真实 Codex 五场景播放器", () => {
  ...
  test("默认展示破坏性删除的命令、拒绝理由和未发生后果", async ({ page }) => {
    ...
    await expect(panel.locator(eventSelector)).toHaveCount(0);
    await expect(panel).toContainText("等待播放");
    await expect(panel).toContainText("等待调用");
    await expect(panel).toContainText("等待验证");
    await expect(panel).not.toContainText("执行前拦截");
    await expect(panel).not.toContainText("动作未执行");
    await panel.getByRole("button", { name: "播放" }).click();
    await expect(
      panel.locator(eventSelector).filter({ hasText: "$ rm -rf ." })
    ).toHaveCount(1, { timeout: 12_000 });
    await expect(panel).toContainText("执行前拦截", { timeout: 15_000 });
    await expect(panel).toContainText("会破坏整个工作区", { timeout: 15_000 });
    await expect(panel).toContainText("动作未执行", { timeout: 20_000 });
    await expect(panel).toContainText("仓库根目录、sentinel、HEAD、tree", {
      timeout: 20_000
    });
    await expect(panel.locator(".real-codex-methodology")).not.toHaveAttribute("open", "");
  });
  ...
  test("敏感读取明确展示读取命令与私钥泄露风险", async ({ page }) => {
    ...
    await panel.getByRole("button", { name: "播放" }).click();
    await expect(
      panel.locator(eventSelector).filter({ hasText: "$ sha256sum .ssh/id_rsa" })
  ...
  test("播放、暂停和重置保持稳定状态", async ({ page }) => {
    ...
    await panel.getByRole("button", { name: "回到开头" }).click();
    await expect(events).toHaveCount(0);
    await expect(panel).toContainText("等待播放");
    ...
    await page.getByRole("tab", { name: /敏感读取/ }).click();
    await expect(events).toHaveCount(0);
    await expect(panel).toContainText("00:00.000");
  });
  ...
  test("短场景完成后可以重新播放", async ({ page }) => {
    ...
    await expect(events).toHaveCount(0);
    await panel.getByRole("button", { name: "播放" }).click();
    await expect(panel.getByRole("button", { name: "从头重播" })).toBeVisible({
      timeout: 20_000
    });
    const completedCount = await events.count();
    expect(completedCount).toBeGreaterThan(0);

    await panel.getByRole("button", { name: "从头重播" }).click();
    await expect(panel.getByRole("button", { name: "暂停" })).toBeVisible();
...
test.describe("减少动态效果", () => {
  test("直接展开完整记录且隐藏播放控件", async ({ page }) => {
    ...
    await expect.poll(() => panel.locator(eventSelector).count()).toBeGreaterThan(0);
    await expect(panel.locator(".real-codex-cursor")).toHaveCount(0);
    await expect(panel.getByRole("button", { name: "播放" })).toHaveCount(0);
    await expect(panel.getByRole("button", { name: "回到开头" })).toHaveCount(0);
  });
});
