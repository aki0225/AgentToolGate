...
`npm test` 会运行组件、证据解析和安全语义测试；覆盖率门禁重点约束
`src/evaluation/realCodexProof.ts`，阈值保持在 85%。
...
- 首页只展示经校验的静态评估摘要和真实 Codex 预录证据，不包含 synthetic 交互状态机。
...
- 证据区分自动评估、`v0.4.2` 正式发布验收、`v0.3.2` 正式包五场景真实 Codex 验收和
...
- 支持键盘导航、真实 Codex 场景标签页方向键、清晰焦点、`aria-live`、375px～1440px 响应式布局和 `prefers-reduced-motion`。
