#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Trellis local-action-firewall PreToolUse adapter for Codex."""
...
def is_valid_codex_hook_output(output: Any) -> bool:
    ...
    if get_text(specific.get("hookEventName")) != "PreToolUse":
        return False
    if get_text(specific.get("permissionDecision")) not in {"allow", "deny"}:
        return False
    # Codex bridge 不需要改写工具输入；拒绝带 updatedInput 的结果，避免透传原始 payload。
...
def call_agenttoolgate_guard_hook_codex(input_data: dict[str, Any]) -> dict[str, Any] | None:
    try:
        completed = subprocess.run(
            [agenttoolgate_executable(), "guard", "hook", "codex", "--input", "-"],
...
def main() -> int:
    ...
    if status == 0:
        if is_high_risk_offline_target(payload):
            ...
            record_local_pending_audit(repo_root, payload, "ATG offline, local pending audit", True)
            return 0
    elif not decision:
